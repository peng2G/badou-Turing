...
def inputs(data_dir,batch_size,distorted):
    ...
    if distorted !=None:
        ...
        print('Filling queue with %d CIFAR image before starting to train. This will take a few minutes'
              %min_queue_examples)
        ...
        images_test,lables_test = tf.train.batch([float_image,read_input.label],batch_size = batch_size,
                                                num_threads=16,capacity=min_queue_examples+3*batch_size)
        return images_test,tf.reshape(lables_test,[batch_size])
