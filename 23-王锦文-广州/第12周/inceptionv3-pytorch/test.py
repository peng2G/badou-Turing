...
def test(args):
    ...
    # 创建模型
    model = build_InceptionV3(args.numclasses)
    ...
    preds=model(inp_img).detach().cpu()
   # print(preds)
    preds=torch.softmax(preds,dim=1)
   # print("preds:",preds.shape,preds)
    predlabel=np.argmax(preds,1).item()
    print("predlabel:",classes[predlabel])
...
if __name__=='__main__':
    ...
    parser.add_argument('--imgpath', default='./test/cat1.jpg', type=str,
